...
const TimelineNode = ({ number, isFirst, isLast }: TimelineNodeProps) => {
    ...
    return (
        <div className="relative col-start-5 col-end-6 mr-7 md:mx-auto">
            ...
            <div className="absolute w-10 h-10 -mt-5 -ml-2 flex items-center justify-center text-primary-200 font-bold border bg-background rounded-full top-1/2 z-10">
                {number}
            </div>
        </div>
    );
...
export const TimelineItem = ({ data, number, isReversed, isFirst, isLast }: TimelineItemProps) => {
    return (
        <div className={`flex md:contents ${isReversed ? "flex-row-reverse" : ""}`}>
            ...
            <div className={`relative gap-xl p-6 my-6 border bg-gray-50 border-gray-300 dark:bg-gray-950 dark:border-gray-800 rounded-xl w-full 
                ...
                <div className="flex flex-col md:flex-row md:justify-between items-start gap-4">
                    ...
                    <Image src={data.logo} alt={data.company} height={40} className="object-contain" />
                </div>
                <p className="text-neutral-400 text-sm md:text-md mt-4 leading-relaxed">{data.description}</p>
            </div>
